"""Telethon-to-dict converters for agent tools.

Simplified version that returns plain dicts instead of Pydantic models,
since agent tools only need serializable data for the LLM.
"""
...
def dialog_to_chat(dialog) -> dict:
    ...
    last_msg = None
    if dialog.message:
        last_msg = message_to_dict(dialog.message, dialog.entity.id)
...
def message_to_dict(msg, chat_id: int) -> dict:
    sender_name = ""
    sender_id = None
    if msg.sender:
